...
export const chatHandlers = [
	http.get(getHost()+'/v1/chat/sessions', async () => {
	...
	http.get(getHost()+'/v1/chat/sessions/:sessionId/messages', async () => {
		return HttpResponse.json({
			message : null,
			data : [
			{
			...
			{
				content : "Hi, I'm fine.",
				subContent: "안녕하세요. 잘지냅니다.",
				sender : "USER"
			},
		]})
	}),
	http.get(getHost()+'/v1/chat/sessions/:sessionId', async ({params}) => {
